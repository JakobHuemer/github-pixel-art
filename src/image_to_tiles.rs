...
pub fn convert_image_to_tiles(year: i32, path: &str) -> Vec<u8> {
    let image_name = path;
    let img = ImageReader::open(image_name)
    ...
    if (img.height() != 7 || img.width() >= 50) {
        panic!(
            "Image dimensions are wrong or too large: w ({}), h ({}) but should be h (7), w (< 50)",
            img.width(),
            img.height()
        )
    }
    ...
    let pixel_data: Vec<u8> = img
        .as_raw()
        .iter()
        .map(|x| ((*x as f64 / 255f64) * 4f64) as u8)
        .collect();

    let _opacities = [" ", ".", "o", "O", "X"];

    let is_leap_year = (year % 4 == 0 && year % 100 != 0) || year % 400 == 0;
    let days_of_year = if is_leap_year { 366 } else { 365 };

    let _date: (i32, i32, i32) = day_to_date(year, 30);
...
pub fn generate_commit_dates(year: i32, tiles_data: &Vec<u8>) -> Vec<String> {
    let mut dates: Vec<String> = Vec::new();

    for (_i, c) in tiles_data.iter().enumerate() {
        for i in 0..*c as u8 {
            let date = day_to_date(year, i as i32);
            dates.push(format!("{:0>4}-{:0>2}-{:0>2}", year, date.1, date.2));
        }
    }
